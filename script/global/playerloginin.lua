...
function main(ExchangeComing)
	--·¢ËÍÉÏ´ÎµÇÂ½µÄĞÅÏ¢¸ø¿Í»§¶Ë,Èç¹ûÊÇµÚÒ»´ÎµÇÂ½µÄ»°
...
--	SB_OnPlayerLogin();
	...
	-- add GM item Test
	if GetItemCount(2, 1, 30644) < 1 then
		AddItem(2, 1, 30644, 1)
		Msg2Player("B¹n nhËn ®­îc 1 quyÓn GM Item!")
	end	
	-- end
...
--	end;
	...
	local nFixTitleDate = tonumber(date("%Y%m%d"))
	if nFixTitleDate >= 20160912 and nFixTitleDate <= 20160915 then
		if IsTitleExist(61,05) <= 0 and gf_GetTransLevel() >= 690 and gf_GetTransLevel() < 700 then
			if AddTitle(61,05) > 0 then
				SetCurTitle(61,05)
				Msg2Player("C¸c h¹ võa nhËn ®­îc danh hiÖu Tø Linh Phôc Sinh ")
			end
		end
		
		if IsTitleExist(61,06) <= 0 and gf_GetTransLevel() >= 773 and gf_GetTransLevel() < 800 then
			if AddTitle(61,06) > 0 then
				SetCurTitle(61,06)
				RemoveTitle(61,05)
				Msg2Player("C¸c h¹ võa nhËn ®­îc danh hiÖu CS7 ")
			end
		end
	...
	elseif GetLevel() >= 80 and GetTask(689) == 0 then	--µÈ¼¶´óÓÚµÈÓÚ80¼¶£¬µÚÒ»´ÎµÇÂ½
		local tbBWSay = {
					--"ÎÒÒªÈ¥Õç×Óµ¤´¦±¨Ãû²ÎÈü/go_to_biwudahui",
...
function LsTmz_login_talk(nTalkSeq)
	Talk(1,"",tLsTmz_TALK_INFO[nTalkSeq][2])
	LsTmzTalkFinish(nTalkSeq)
end
